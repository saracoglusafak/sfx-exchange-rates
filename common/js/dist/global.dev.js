...
function sfxexchangerates_serialized(obj, clear) {
  ...
  if (clear) _return = _return.replace(/^\&(.*)$/g, '$1');
  return _return;
}
...
function sfxexchangerates_idGenerate(how, prefix, lower, upper, number) {
  ...
  if (typeof number == "undefined") number = true; // console.log(lower);

  var text = "";
  var possible = "";
  if (lower) possible += "abcdefghijklmnopqrstuvwxyz";
  if (upper) possible += "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  if (number) possible += "0123456789"; // console.log(possible);
